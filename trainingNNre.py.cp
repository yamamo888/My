...
if __name__ == "__main__":
    ...
    if isRegression:
        ...
        loss_test = mytraining.Loss(y,predict_test_op)
        ...
        for i in range(nTraining):
            ...
            if i % testPeriod == 0:
                # regression(範囲を狭く)
                testRegLoss,predTestex,TestY = sess.run([loss_test,predict_test_op,y],feed_dict={x:testX,y1_:testY1,y2_:testY2,y3_:testY3})
                ...
                print("itr:%d,trainRegLoss:%f,testRegLoss:%f" % (i,trainRegLoss,testRegLoss))
                print("------------------------------------")
                print('testb1mean',np.mean(np.abs(testY1-predTest[:,0][:,np.newaxis]))) 
                print('testb1Var',np.var(np.abs(testY1-predTest[:,0][:,np.newaxis])))
                print("------------------------------------")
                print('testb2mean',np.mean(np.abs(testY2-predTest[:,1][:,np.newaxis]))) 
                print('testb2Var',np.var(np.abs(testY2-predTest[:,1][:,np.newaxis])))
                print("------------------------------------")
                print('testb3mean',np.mean(np.abs(testY3-predTest[:,2][:,np.newaxis]))) 
                print('testb3Var',np.var(np.abs(testY3-predTest[:,2][:,np.newaxis])))
                print("------------------------------------")
                print('b1mean',np.mean(np.abs(batchY1-predTrain[:,0][:,np.newaxis]))) 
                print('b1Var',np.var(np.abs(batchY1-predTrain[:,0][:,np.newaxis])))
                print("------------------------------------")
                print('b2mean',np.mean(np.abs(batchY2-predTrain[:,1][:,np.newaxis]))) 
                print('b2Var',np.var(np.abs(batchY2-predTrain[:,1][:,np.newaxis])))
                print("------------------------------------")
                print('b3mean',np.mean(np.abs(batchY3-predTrain[:,2][:,np.newaxis]))) 
                print('b3Var',np.var(np.abs(batchY3-predTrain[:,2][:,np.newaxis])))
                ...
                with open('./visualization/residual/TestR_{}_{}_{}_{}.pickle'.format(i,CellMode,depth,reduce_val),'wb') as fp:
                ...
                with open('./visualization/residual/TrainR_{}_{}_{}_{}.pickle'.format(i,CellMode,depth,reduce_val),'wb') as fp:
                        pickle.dump(predTrain[:,0],fp)
                        pickle.dump(predTrain[:,1],fp)
                        pickle.dump(predTrain[:,2],fp)
                        pickle.dump(batchY1,fp)
                        pickle.dump(batchY2,fp)
                        pickle.dump(batchY3,fp)
        
    
    myPlot = Plot(isPlot=isPlot ,nClass=nClass, alpha=depth) 
    myPlot.Plot_loss(trainRegLosses, testRegLosses, testPeriod)
