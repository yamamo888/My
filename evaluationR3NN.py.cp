...
class Trainingeqp():
    ...
    def ClassCenter_Residual(self,y_class,predict,start_center,isEval=False):
        # 予測したクラスの最大クラス
...
if __name__ == "__main__":
    ...
    if isEval:
        ...
        #testX = np.reshape(np.tile(testX,100),[-1,50])
    ...
    if methodModel == 1:
        ...
        pred_class1_center_test = mytraining.ClassCenter_Residual(y1_class,predict_class1_test_op,sCent_nankai,isEval=True)
        pred_class2_center_test = mytraining.ClassCenter_Residual(y2_class,predict_class2_test_op,sCent,isEval=True)
        pred_class3_center_test = mytraining.ClassCenter_Residual(y3_class,predict_class3_test_op,sCent,isEval=True)
        ...
        for i in range(nTraining):
            ...
            if i % testPeriod == 0:
                if isEval:
                    testClassCenter = sess.run(class_center_all_test,feed_dict={x:testX})    
                    # regression
                    TestPred = sess.run(predict_regression_residual_test_op,feed_dict={x:testX})
                ...
                if isEval:
                    ...
                    with open('./visualization/eval/TestATR_{}_{}_{}_{}.pickle'.format(i,classMode,depth,10),'wb') as fp:
        ...
        alpha_test_op = mytraining.Alpha(reuse=True)
        ...
        reduce_residual_test =  mytraining.Reduce(predict_regression_residual_op,alpha_test_op)
        ...
        for i in range(nTraining):
            ...
            if i % testPeriod == 0:
                ...
                if isEval:
                    ...
                    TestPred = sess.run(predict_regression_residual_test_op,feed_dict={x:testX})
                    # alpha
                    TestAlpha = sess.run(alpha_test_op,feed_dict={x:testX})
                    # Reduce
