    ...
    for tfID in [190]:
        ...
        for iS in np.arange(gtJ.shape[0]):
            ...
            for fID in np.arange(len(files)):
                ...
                if iS == 0:
                    if cell == 2 or cell == 4 or cell == 5:
                        ...
                        yth, yV, pJ_all, maxSim = myData.MinErrorNankai(gtV,yU,yth,yV,pJ_all,cell=cell,nCell=nCell)
            ...
            if cell == 2 or cell == 4 or cell == 5:
                ...
                # ※perticle分？
                for i in np.arange(np.max([kInds[ntI].shape[0],kInds[tntI].shape[0],kInds[ttI].shape[0]])): # 最大回数で
                    # yth_all [1400,8(cell),perticle]
                    tmpth_nk = yth_all[kInds[ntI],ntI,i]
                    tmpth_tnk = yth_all[kInds[tntI],ntI,i]
                    tmpth_tk = yth_all[kInds[ttI],ttI,i]
                    # V
                    tmpV_nk = yV_all[kInds[ntI],ntI,i]
                    tmpV_tnk = yV_all[kInds[tntI],tntI,i]
                    tmpV_tk = yV_all[kInds[ttI],ttI,i]
                    
                    if not flag2:
                        yth_rYear_nk = tmpth_nk
                        yth_rYear_tnk = tmpth_tnk
                        yth_rYear_tk = tmpth_tk
                        
                        yV_rYear_nk = tmpV_nk
                        yV_rYear_tnk = tmpV_tnk
                        yV_rYear_tk = tmpV_tk
                        
                        flag2 = True
                    else:
                        yth_rYear_nk = np.concatenate([yth_rYear_nk,tmpth_nk])
                        yth_rYear_tnk = np.concatenate([yth_rYear_tnk,tmpth_tnk])
                        yth_rYear_tk = np.concatenate([yth_rYear_tk,tmpth_tk])
                        
                        yV_rYear_nk = np.concatenate([yV_rYear_nk,tmpV_nk])
                        yV_rYear_tnk = np.concatenate([yV_rYear_tnk,tmpV_tnk])
                        yV_rYear_tk = np.concatenate([yV_rYear_tk,tmpV_tk])
                #yth_rYear = np.concatenate((yth_rYear_nk,yth_rYear_tnk,yth_rYear_nk),1)
                #yV_rYear = np.concatenate((yV_rYear_nk,yV_rYear_tnk,yV_rYear_nk),1)
                pdb.set_trace()
                # for nankai,tonankai,tokai
                yth_rYear_nk = resampled[thInd,:,ntI]
                yth_rYear_tnk = resampled[thInd,:,tntI]
                yth_rYear_tk = resampled[thInd,:,ttI]
                
                yV_rYear_nk = resampled[vInd,:,ntI]
                yV_rYear_tnk = resampled[vInd,:,tntI]
                yV_rYear_tk = resampled[vInd,:,ttI]
