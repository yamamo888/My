    ...
    for tfID in [190]:
        ...
        for iS in np.arange(ptime): # gt times
            ...
            for fID in np.arange(nP):
                ...
                _, _, _, pJ_all, _, _ = myData.MSErrorNankai(gtV,yU,yth,yV,pJ_all,nCell=nCell)
                
                # concatするために長さそろえる
                nkJ = np.pad(pJ_all[0],(0,30-pJ_all[0].shape[0]),"constant",constant_values=0)
                tnkJ = np.pad(pJ_all[1],(0,30-pJ_all[1].shape[0]),"constant",constant_values=0)
                tkJ = np.pad(pJ_all[2],(0,30-pJ_all[2].shape[0]),"constant",constant_values=0)
                # eq. years [100,3(cell)]
            ...
            #pdb.set_trace()
            print("---- Start PF !! ----\n")
            # resampledB: [perticles(B),3(cell)]
            # updateID: resampled index
            resampledB, updateID = sp.simulate(Xt,gtJs,pJs,mode=mode,t=iS,pTime=ptime,nP=nP,nCell=nCell,isSavetxt=isSavetxt,isPlot=isPlot)
            # --------------------------------------------------------------- #
            # Call simulation (X_t -> X_t+1)
            updateX_not.updateParameters(resampledB)