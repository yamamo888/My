...
class DeepModel():
    ...
    def fc_relu_gl(self,inputs,w,keepProb):
         relu = tf.matmul(inputs,w) 
         relu = tf.nn.dropout(relu, keepProb)
         relu = tf.nn.relu(relu) 
         return relu
    ...
    def fc_gl(self,inputs,w,keepProb):
    ...
    def Regression(self,x,reuse=False,isRegression=False,name_scope="Regression",depth=0):
    ...
    def GroupLassoRegression(self,x,reuse=False,name_scope="GLRegression"):
        with tf.variable_scope(name_scope) as scope:  
            keepProb = 1.0
            if reuse:
                keepProb = 1.0            
            ...
            w1_glregression = self.weight_variable('w1_glregression',[self.nInputReg,self.nRegHidden])
            bias1_glregression = self.bias_variable('bias1_glregression',[self.nRegHidden])

            fc1 = self.fc_relu(x,w1_glregression,bias1_glregression,keepProb) 
            
            w2_glregression = self.weight_variable('w2_glregression',[self.nRegHidden,self.nRegHidden2])
            bias2_glregression = self.bias_variable('bias2_glregression',[self.nRegHidden2])
            
            fc2 = self.fc_relu(fc1,w2_glregression,bias2_glregression,keepProb)
            
            w3_glregression = self.weight_variable('w3_glregression',[self.nRegHidden2,self.nRegHidden3])
            bias3_glregression = self.bias_variable('bias3_glregression',[self.nRegHidden3])
            
            y_regression = self.fc(fc2,w3_glregression,bias3_glregression,keepProb)  
            
            return y_regression,w2_glregression,w3_glregression
...
class Plot():
    ...
    Predicted value
    """
    def __init__(self,isPlot=False,isRR=False,methodModel=1,sigma=0.1,nClass=1,alpha=1,pNum=6, nTrain=0, nTest=0):
        self.isPlot = isPlot
        self.isRR = isRR
    ...
    def Plot_3D(self,x1Test,x2Test,yTest,yPred,isTrain=0):
        if self.isPlot:
            ...
            plt.legend()
            fullPath = os.path.join(self.visualPath,"Pred_{}_{}_{}_{}_{}_{}_{}_{}.png".format(self.methodModel,sigma,self.nClass,self.alpha, self.pNum, self.nTrain, self.nTest, isTrain))
    ...
    def Plot_loss(self,trainTotalLosses, testTotalLosses, trainClassLosses, testClassLosses,trainRegLosses, testRegLosses, testPeriod):
        if self.isPlot:
            if self.methodModel==2 or self.methodModel==1:
                ...
                plt.legend()
                fullPath = os.path.join(self.visualPath,"Loss_{}_{}_{}_{}_{}_{}_{}.png".format(self.methodModel,sigma,self.nClass,self.alpha, self.pNum, self.nTrain, self.nTest))
            else:
                ...
                plt.legend()
                fullPath = os.path.join(self.visualPath,"Loss_{}_{}_{}_{}_{}_{}.png".format(self.methodModel,sigma,self.nClass,self.alpha, self.pNum, self.nTrain, self.nTest))
...
if __name__ == "__main__":
    ...
    nTrain = 1000
    nTest = 7000
    dataPath = "toyDataFile"
    toydataPath = "toyData_{}_{}_{}_{}.png".format(sigma,pNum,nTrain,nTest)
    ...
    if methodModel == 0:
        predict_regression_op = myDeepModel.Regression(x,isRegression=True,depth=depth)
        predict_regression_test_op = myDeepModel.Regression(x,reuse=True,isRegression=True,depth=depth)
        ...
        for i in range(nTraining):
            ...
            if i % testPeriod == 0:
                ...
                if not flag:
                    ...
                    #trainRegL2Losses,testRegL2Losses = RegL2Loss[np.newaxis],testRegL2Loss[np.newaxis]
                    flag = True
                else:
                    trainRegLosses,testRegLosses = np.hstack([trainRegLosses,RegLoss[np.newaxis]]),np.hstack([testRegLosses,testRegLoss[np.newaxis]])
                    #trainRegL2Losses,testRegL2Losses = np.hstack([trainRegL2Losses,RegL2Loss[np.newaxis]]),np.hstack([testRegL2Losses,testRegL2Loss[np.newaxis]])
    ...
    # Not Truncate
        ...
        predict_regression_op = myDeepModel.Regression(classCenter_x,isRegression=True)
        predict_regression_test_op = myDeepModel.Regression(classCenter_x_test,reuse=True,isRegression=True)
        ...
        for i in range(nTraining):
            ...
            if i % testPeriod == 0:
                ...
                print(yTest[:10])
                print("---------------")
                print("itr:%d,trainClsLoss:%f,trainRegLoss:%f, trainTotalLoss:%f" % (i,trainClassLoss,trainRegLoss, trainTotalLoss))
                print("-----------------------------------")
                print("itr:%d,testClsLoss:%f,testRegLoss:%f, testTotalLoss:%f" % (i,testClassLoss,testRegLoss, testTotalLoss)) 
                #-----------------------------------------------------------------------
                if not flag:
    ...
    # MVR
        ...
        predict_regression_op = myDeepModel.Regression(classCenter_x)
        predict_regression_test_op = myDeepModel.Regression(classCenter_x_test,reuse=True)
        ...
        for i in range(nTraining):
            ...
            if i % testPeriod == 0:
                ...
                print(yTest[:10])
                print("---------------")
                print("itr:%d,trainMagLoss:%f,trainRegLoss:%f, trainTotalLoss:%f" % (i,trainClassLoss,trainRegLoss, trainTotalLoss))
                print("-----------------------------------")
                print("itr:%d,testMagLoss:%f,testRegLoss:%f, testTotalLoss:%f" % (i,testClassLoss,testRegLoss, testTotalLoss)) 
                #-----------------------------------------------------------------------
                if not flag:
                    trainRegLosses,testRegLosses = trainRegLoss[np.newaxis],testRegLoss[np.newaxis]
                    trainClassLosses,testClassLosses = trainClassLoss[np.newaxis],testClassLoss[np.newaxis]
    ...
    if methodModel == 2:
        # 可視化
        myPlot = Plot(isPlot=isPlot,methodModel=methodModel,sigma=sigma, nClass=nClass, alpha=TestAlpha, pNum=pNum, nTrain=nTrain, nTest=nTest)
        ...
        myPlot = Plot(isPlot=isPlot,methodModel=methodModel,sigma=sigma, nClass=nClass, alpha=0, pNum=pNum, nTrain=nTrain, nTest=nTest)
